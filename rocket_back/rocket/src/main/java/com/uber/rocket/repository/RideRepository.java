...
public interface RideRepository extends PagingAndSortingRepository<Ride, Long> {
    ...
    @Query("FROM Ride ride WHERE ride.vehicle = :vehicle AND ride.status IN (:confirmed,:started)")
    Ride findRideByVehicleAndStatus(@Param("vehicle") Vehicle vehicle, @Param("confirmed") RideStatus confirmed, @Param("started") RideStatus started);
